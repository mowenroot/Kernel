...
static void maybe_add_auto(u8* mem, u32 len) {
  ...
  if (a_extras_cnt < MAX_AUTO_EXTRAS) {
    ...
    //词典最多500条，如果满了随机从250-500中随机覆盖一个
    i = MAX_AUTO_EXTRAS / 2 +
  ...
  for (i = 0; i < len; i++) {
    ...
    for (j = 0; j < MIN(a_extras_cnt, USE_AUTO_EXTRAS); j++) {
      ...
      */
      ...
      //测试 直接覆盖原有数据，因为a_extras会按len升序排序的，所以不用还原out_buf
      if (common_fuzz_stuff(argv, out_buf, len)) goto abandon_entry;
...
/* 检查CPU频率调节器(governor)设置
   主要检查:
   1. CPU是否使用performance模式而不是ondemand模式
   2. CPU最小/最大频率是否一致
   3. 如果使用ondemand模式且最大最小频率不一致,会提示用户切换到performance模式
   
   原因:
   1. ondemand模式下CPU频率动态调节会影响性能测量的准确性
   2. 需要稳定的CPU频率以准确测量执行时间
   3. fuzzing过程需要持续的高性能
   
   可以通过设置AFL_SKIP_CPUFREQ环境变量跳过此检查 */
...
/* 设置信号处理函数。
   这个函数比看起来要复杂,主要是因为在Solaris上的libc有一些特殊行为:
   - 不会自动恢复被中断的read()调用
   - 调用siginterrupt()时会设置SA_RESETHAND标志
   - 还有其他一些不必要的行为
   所以需要特别处理信号相关的设置 */

EXP_ST void setup_signal_handlers(void) {

  /* 定义sigaction结构体,用于设置信号处理方式 */
  struct sigaction sa;

  /* 初始化sigaction结构体
     - sa_handler设为NULL表示使用默认处理
     - SA_RESTART标志使被信号中断的系统调用自动重启
     - sa_sigaction设为NULL因为我们使用sa_handler而不是sa_sigaction */
  sa.sa_handler   = NULL;
  sa.sa_flags     = SA_RESTART;
  sa.sa_sigaction = NULL;

  /* 清空信号掩码,表示在处理信号时不阻塞任何其他信号 */
  sigemptyset(&sa.sa_mask);

  /* 设置终止相关的信号处理
     SIGHUP(挂起)、SIGINT(中断)、SIGTERM(终止)都由handle_stop_sig处理 */
...
int main(int argc, char** argv) {
  ...
  /* 是否只执行一次就退出,由环境变量AFL_BENCH_JUST_ONE控制 */
  ...
  /* 设置目录和文件描述符 */
  /* 设置输出目录和文件描述符
     1. 创建输出目录结构(queue/crashes/hangs等)
     2. 设置文件权限
     3. 初始化文件描述符 */
  setup_dirs_fds();
  ...
  /* 加载之前保存的状态
     1. 读取fuzzer_stats文件获取统计信息
     2. 恢复上次运行的进度
     3. 加载已发现的路径信息 */
  load_auto();

  /* 重新组织输入队列
     1. 根据文件大小和执行时间对测试用例排序
     2. 优化测试用例的执行顺序
     3. 为确定性变异阶段做准备 */
  pivot_inputs();
  ...
  /* 检测目标程序的命令行参数中是否包含@@等文件参数标记
     - argv + optind + 1跳过程序名,检查后续参数
     - 用于确定如何传递测试用例给目标程序 */
  ...
  while (1) {
    ...
    /* 调用fuzz_one() 对当前输入进行fuzzing，返回1跳过当前用例*/
  ...
  /* 如果是程序停止,则杀死forkserver和当前运行的进程 */
  if (stop_soon == 2) {
      if (child_pid > 0) kill(child_pid, SIGKILL);
      if (forksrv_pid > 0) kill(forksrv_pid, SIGKILL);
  }
  /* 等待forkserver结束以获取资源使用统计 */
